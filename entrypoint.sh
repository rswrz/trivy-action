...
if [ "${format}" == "sarif" ] && [ "${limitSeveritiesForSARIF}" != "true" ]; then
  ...
  echo "::debug::Building SARIF report with options: ${SARIF_ARGS}" "${artifactRef}"  | sed 's/\\n/\n/g'
  trivy --quiet ${scanType} --format sarif --output ${output} $SARIF_ARGS ${artifactRef}
elif [ $trivyConfig ]; then
   echo "::debug::Running Trivy with trivy.yaml config from: " $trivyConfig  | sed 's/\\n/\n/g'
   trivy --config $trivyConfig ${scanType} "${artifactRef}"
else
   echo "::debug::Running trivy with options: trivy ${scanType}" "${ARGS[@]}" "${artifactRef}" | sed 's/\\n/\n/g'
   echo "::debug::Global options: " "${GLOBAL_ARGS}" | sed 's/\\n/\n/g'
